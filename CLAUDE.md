...
| `*`    | `/pay/:token/3ds` | Simulated ACS (Phase 2). |
...
| `4000 0000 0000 3220` | Requires 3DS challenge | `initiated` → challenge (Phase 2) |
...
**Phase 2** — simulated 3DS challenge (redirect to `/pay/:token/3ds`, an ACS page with Approve/Decline, then back); partial captures and refunds; `void`/`expire` flows; webhook retries with exponential backoff; a merchant dashboard listing payments and their status timeline.
